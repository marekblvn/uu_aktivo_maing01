...
const BeingInitializedAuthorized = createVisualComponent({
  ...
  render() {
    ...
    const { data: uuAktivo } = useSubAppData();
    ...
    if (uuAktivo?.temporaryData?.dtoIn.mode) {
      uuAktivo.temporaryData.dtoIn.mode = "retry";
    }

    const [showConsole, setShowConsole] = useState(!!uuAktivo.consoleCode);
    const [retryInputData, setRetryInputData] = useState(
      JSON.stringify({ force: false, mode: "retry", data: uuAktivo?.temporaryData?.dtoIn }, null, 2),
    );
    const [rollbackInputData, setRollbackInputData] = useState(
      JSON.stringify({ force: true, mode: "rollback", data: {} }, null, 2),
    );
    const consoleBaseUri = uuAktivo.temporaryData.progressMap.uuConsoleUri;
    const progressCode = uuAktivo.temporaryData.progressMap.progressCode;
    const consoleCode = uuAktivo.temporaryData.progressMap.consoleCode;
