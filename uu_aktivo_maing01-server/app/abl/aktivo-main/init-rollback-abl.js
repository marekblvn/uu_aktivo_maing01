...
class InitRollbackAbl {
  ...
  async initRollback(appUri, configuration, lockSecret) {
    ...
    return await scriptEngineClient.runScript({
      scriptCode: "uu_aktivo_maing01-uuscriptlib/aktivo-main/init-rollback",
      scriptDtoIn,
    });
  }
  ...
  async _initFinalizeRollback(uri, dtoIn) {
    ...
    if (!uuAktivo) {
      // 3.1
      throw new Errors._initFinalizeRollback.UuAktivoDoesNotExist({ awid });
    }

    if (![AktivoMainConstants.StateMap.BEING_INITIALIZED, AktivoMainConstants.StateMap.CREATED].includes(uuAktivo.state)) {
      // 3.2
      throw new Errors._initFinalizeRollback.NotInProperState({
        state: uuAktivo.state,
        expectedStateList: [AktivoMainConstants.StateMap.BEING_INITIALIZED, AktivoMainConstants.StateMap.CREATED],
      });
    }

    // HDS 4
    const sysIdentitySession = await AuthenticationService.authenticateSystemIdentity();
    const { uuConsoleUri, progressCode, consoleCode } = uuAktivo.temporaryData.progressMap;
    let progressClient = null;
    if (
      !uuAktivo.temporaryData.rollbackStepList.includes(AktivoMainConstants.InitRollbackStepMap.PROGRESS_DELETED.code)
    ) {
    ...
    const stepHandler = new StepHandler({
      ...
      progressClient,
      stepList: uuAktivo.temporaryData.rollbackStepList,
      rollbackMode: true,
    });
    ...
    // HDS 5
    if (uuAktivo.temporaryData.stepList.includes(AktivoMainConstants.InitStepMap.CONSOLE_CREATED.code)) {
      uuAktivo = await stepHandler.handleStep(
        uuAktivo,
        AktivoMainConstants.InitRollbackStepMap.CONSOLE_CLEARED,
        async () => {
    ...
    // HDS 6
    if (uuAktivo.temporaryData.stepList.includes(AktivoMainConstants.InitStepMap.WS_CONNECTED.code)) {
      uuAktivo = await stepHandler.handleStep(
        uuAktivo,
        AktivoMainConstants.InitRollbackStepMap.WS_DISCONNECTED,
        async () => {
    ...
    // HDS 7
    if (uuAktivo.temporaryData.stepList.includes(AktivoMainConstants.InitStepMap.AWSC_CREATED.code)) {
      await stepHandler.handleStep(uuAktivo, AktivoMainConstants.InitRollbackStepMap.AWSC_DELETED, async () => {
        await this._deleteAwsc(uuAktivo, uri, sysIdentitySession);
      });
    }
    ...
    await stepHandler.handleStep(
      uuAktivo,
      AktivoMainConstants.InitRollbackStepMap.PROGRESS_DELETED,
      async () => {
        try {
          await progressClient.end({
            state: ProgressConstants.StateMap.COMPLETED,
            message: "Rollback finished.",
            doneWork: AktivoMainConstants.getInitRollbackStepCount(),
          });
        } catch (e) {
  ...
  async _deleteAwsc(uuAktivo, appUri, session) {
    const appClientOpts = { baseUri: uuAktivo.uuTerritoryBaseUri, appUri, session };

    try {
      await UuTerritoryClient.Awsc.setState(
        {
          id: uuAktivo.artifactId,
          state: AktivoMainConstants.StateMap.FINAL,
        },
        appClientOpts,
      );
    } catch (e) {
      ...
      switch (e.code) {
        ...
        case TerritoryConstants.INVALID_ARTIFACT_STATE:
          if (e.paramMap?.artifactState === AktivoMainConstants.StateMap.FINAL) {
            // 5.1.2.
            throwError = false;
            DtoBuilder.addWarning(new Warnings._initFinalizeRollback.UuAwscDoesNotExist());
          }
      ...
      if (throwError) {
        // 5.1.3.
        throw new Errors.AktivoMain.SetAwscStateFailed({}, e);
      }
    }

    try {
      await UuTerritoryClient.Awsc.delete({ id: uuAktivo.artifactId }, appClientOpts);
    } catch (e) {
      if (e.code === TerritoryConstants.ARTIFACT_DOES_NOT_EXIST) {
        ...
        // 5.2.2.
        throw new Errors.AktivoMain.DeleteAwscFailed({}, e);
      }
    }
  }
}
