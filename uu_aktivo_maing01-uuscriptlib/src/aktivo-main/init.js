...
class AktivoMainInitClient {
  ...
  async initFinalize() {
    return this.aktivoMainClient.initFinalize(this.lockSecret);
  }
}
/*@@viewOff:scriptClient*/

/*@@viewOn:validateDtoIn*/
const DtoInValidationSchema = `const scriptAktivoMainInitDtoInType = shape({
...
function validateDtoIn(dtoIn, uuAppErrorMap = {}) {
  ...
  let validationResult = dtoInValidator.validate("scriptAktivoMainInitDtoInType", dtoIn);
...
async function main() {
  await console.info(`Script uuAktivo init started`);
  dtoOut.dtoIn = dtoIn;
  ...
  let mainContext = new AktivoMainInitClient(dtoIn.lockSecret);
  ...
  uuAktivo = await mainContext.initFinalize();
